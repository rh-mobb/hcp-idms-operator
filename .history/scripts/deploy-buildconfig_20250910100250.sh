...
oc start-build hcp-idms-operator --from-file=manager --follow
...
echo "  GOOS=linux GOARCH=amd64 make manager && cp bin/manager manager && oc start-build hcp-idms-operator --from-file=manager --follow"
