...
func (r *ImageMirrorDigestSetReconciler) generateRegistryConfig(instance *configv1alpha1.ImageMirrorDigestSet) error {
	...
	if err := r.reloadCRIO(); err != nil {
		log.Error(err, "Failed to reload CRI-O", "configFile", configFile)
		// Don't return error here as the config file was written successfully
		// CRI-O will pick up the changes on its next check
	} else {
		log.Info("Successfully reloaded CRI-O", "configFile", configFile)
	}
...
func (r *ImageMirrorDigestSetReconciler) reloadCRIO() error {
	...
	if err := cmd.Run(); err != nil {
		...
		log.Info("systemctl reload failed, trying pkill approach", "error", err)
