...
func (r *ImageMirrorDigestSetReconciler) generateRegistryConfig(instance *configv1alpha1.ImageMirrorDigestSet) error {
	...
	// Create a signal file to trigger CRI-O reload
	if err := r.createCRIOReloadSignal(); err != nil {
		// Don't fail the reconciliation for this, just return the error
		return fmt.Errorf("failed to create CRI-O reload signal: %w", err)
	}
...
// createCRIOReloadSignal creates a signal file to trigger CRI-O reload
func (r *ImageMirrorDigestSetReconciler) createCRIOReloadSignal() error {
	// Create a signal file in a more appropriate location for runtime files
	signalDir := "/var/run/hcp-idms-operator"
	signalFile := filepath.Join(signalDir, "crio-reload-needed")
	...
	// Write a timestamp to the signal file
	timestamp := time.Now().Format(time.RFC3339)
	content := fmt.Sprintf("# CRI-O reload signal\n# Generated at: %s\n# This file indicates that CRI-O should be reloaded to pick up registry configuration changes\n# Location: %s\n", timestamp, signalFile)
	
	if err := os.WriteFile(signalFile, []byte(content), 0644); err != nil {
		return fmt.Errorf("failed to create CRI-O reload signal file: %w", err)
	}
