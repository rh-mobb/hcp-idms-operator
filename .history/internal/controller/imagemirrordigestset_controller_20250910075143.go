...
func (r *ImageMirrorDigestSetReconciler) generateRegistryConfig(instance *configv1alpha1.ImageMirrorDigestSet) error {
	...
	if err := r.createCRIOReloadSignal(); err != nil {
		log.Error(err, "Failed to create CRI-O reload signal", "file", configFile)
		// Don't fail the reconciliation for this, just log the error
	} else {
		log.Info("Registry configuration updated, CRI-O reload signal created", 
			"file", configFile, "resource", instance.Name)
	}
...
func (r *ImageMirrorDigestSetReconciler) createCRIOReloadSignal() error {
	// Create a signal file that the CRI-O reload monitor will watch
	signalFile := "/etc/containers/registry.conf.d/.crio-reload-needed"
