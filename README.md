...
This operator watches for `ImageDigestMirrorSet` custom resources and generates registry configuration files in `/etc/containers/registries.conf.d/` on each node. It ignores resources named "default" and creates individual configuration files for each remaining resource.

## Features

- **DaemonSet Deployment**: Runs on every node in the cluster
- **Resource Filtering**: Automatically ignores "default" and Hypershift-managed ImageDigestMirrorSet resources
...
1. **Custom Resource Definition**: `ImageDigestMirrorSet` CRD
2. **Controller**: Watches and processes ImageDigestMirrorSet resources
3. **DaemonSet**: Deploys the operator on every node
4. **RBAC**: Proper permissions for cluster-wide resource access

## Resource Filtering

The operator automatically ignores certain ImageDigestMirrorSet resources to prevent conflicts:
...
For each ImageDigestMirrorSet resource (except "default" and Hypershift-managed), the operator generates a TOML configuration file in `/etc/containers/registries.conf.d/`:
...
To run the operator locally for testing without modifying the actual filesystem:
...
In debug mode, the operator will:
- Log configuration content instead of writing files
- Skip CRI-O reload operations
- Print detailed debug information to stdout

This is useful for:
- Testing configuration generation logic
...
by default openshift blocks any changes to mirror configs, we can tell it to only block if the label shows it as a hypershift managed config.
...
- **Host Path Mount**: `/etc/containers/registries.conf.d` is mounted from the host
...
- **Host Path Access**: Required to write to `/etc/containers/registries.conf.d/`
...
- **ImageDigestMirrorSet Resources**: Full CRUD operations on `config.openshift.io/v1alpha1` resources
...
```bash
# Check ImageDigestMirrorSet resources
oc get imagedigestmirrorsets

# Describe a specific resource
oc describe imagedigestmirrorset quay-mirror

# Check which resources are being ignored
oc get imagedigestmirrorsets -o custom-columns=NAME:.metadata.name,MANAGED:.metadata.labels.hypershift\.openshift\.io/managed
```
...
The operator is automatically built and pushed to `ghcr.io/openshift/hcp-idms-operator` on every push to main.
